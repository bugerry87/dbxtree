...
class MeshMerge:
	...
	def __job__(self):
		...
		while not rospy.is_shutdown():
			...
			if self.area: #reject large faces
				mask = spatial.face_magnitude(verts[Ti]).flatten() <= self.area
				Ti = Ti[mask]
				uid, idx = np.unique(Ti, return_inverse=True)
				srt = np.argsort(uid, axis=None)
				Ti = srt[idx].reshape(-1, 3)
				verts = verts[uid][srt]
				norms = norms[uid][srt]
			...
			if first:
				...
				self.verts = verts
				self.norms = norms
				self.obs = np.zeros(len(verts), dtype=int)
				self.Ti = Ti
				first = False
				continue
			...
			if self.radius:
				...
				idx = np.nonzero(self.obs)
				mask = np.isin(self.Ti, idx)
				self.Ti = self.Ti[np.any(mask, axis=-1)]
			...
			self.obs = np.hstack((self.obs, np.zeros(len(verts))))
...
if __name__ == '__main__':
	...
	def init_argparse(parents=[]):
		...
		parser = ArgumentParser(
			description="ROS node to merge TriangleMesh.",
			parents=parents
			)
